...
//UNSETTERS
export function clear_settings(ractive, gear_string, job_list, job_names, zip){
  ractive.set('results_visible', 1);
  ractive.set('results_panel_visible', 1);
  ractive.set('download_links', '');
  job_list.forEach(function(job_name){
    ractive.set(job_name+'_results_visible', false);
  ...
  ractive.set('dmp_map', null);
  ractive.set('dmpfold_pdb', null);
  ractive.set('dmpfold_tmscore', null);
  ...
  ractive.set('hspred_initial_mol_viewer',null);
  ractive.set('hspred_second_mol_viewer', null);
  ractive.set('metsite_mol_viewer', null);

  ractive.set('annotations',null);
  ractive.set('batch_uuid',null);
  ractive.set('config_text', "job,step,type,name,version,parameters\n");
  ...
  job_list.forEach(function(job_name){
    if(data.job_name === job_name)
    {
      ...
      if(job_name === 'pgenthreader' || job_name === 'dompred'  ||
         ...
         job_name === 'ffpred' || job_name === 'domserf' || job_name === 'bioserf' || job_name === 'dmp')
      {
...
export function handle_results(ractive, data, file_url, zip, downloads_info, job_names, job_list, results_found)
{
  //RESULTS VARS
  ...
  let mempack_graph_out = /graph.out$/;
  ...
  for(let i in results)
  {
    ...
    // s4pred results
    if(result_dict.name == 's4pred_run_model')
    {
      results_found.s4pred = true;
      let match = horiz_regex.exec(result_dict.data_path);
      if(match)
      {
        process_file(file_url, result_dict.data_path, 'horiz', zip, ractive);
        ractive.set("s4pred_waiting_message", '');
        ractive.set("s4pred_waiting_icon", '');
        ractive.set("s4pred_time", '');
        downloads_info.s4pred.horiz = '<a href="'+file_url+result_dict.data_path+'">Horiz Format Output</a><br />';
      }
      let ss2_match = ss2_regex.exec(result_dict.data_path);
      if(ss2_match)
      {
        downloads_info.s4pred.ss2 = '<a href="'+file_url+result_dict.data_path+'">SS2 Format Output</a><br />';
        process_file(file_url, result_dict.data_path, 'ss2', zip, ractive);
      }
    }
    ...
    if(result_dict.name === 'memsatsvm' || result_dict.name === 'memsat-pore')
    {
    ...
    if(result_dict.name === 'runBioserf')
    {
      ...
      process_file(file_url, result_dict.data_path, 'zip', zip, ractive);
      display_structure(file_url+result_dict.data_path, '#bioserf_model', true, ractive);
    ...
    if(result_dict.name === 'contact_predictors')
    {
        ...
        if(ev_match)
        {
          downloads_info.dmp.freecontact = '<a href="'+file_url+result_dict.data_path+'">FreeContact predictions</a><br />';
          process_file(file_url, result_dict.data_path, 'zip', zip, ractive);
        }
        let ps_match = metapsicov_psicov_regex.exec(result_dict.data_path);
        if(ps_match)
        {
          downloads_info.dmp.psicov = '<a href="'+file_url+result_dict.data_path+'">PSICOV predictions</a><br />';
          process_file(file_url, result_dict.data_path, 'zip', zip, ractive);
        }
        let cc_match = metapsicov_ccmpred_regex.exec(result_dict.data_path);
        if(cc_match)
        {
          downloads_info.dmp.ccmpred = '<a href="'+file_url+result_dict.data_path+'">CCMPRED predictions</a><br />';
          process_file(file_url, result_dict.data_path, 'zip', zip, ractive);
        }
    ...
    if(result_dict.name === 'metpred_wrapper')
    {
      ...
      if(pdb_match)
      {
        ...
        display_structure(file_url+result_dict.data_path, '#metsite_model', false, ractive);
        process_file(file_url, result_dict.data_path, 'zip', zip, ractive);
      }
    }
    ...
    if(result_dict.name === 'split_pdb_files')
    {
      ...
      if(initial_match)
      {
          ...
          display_structure(file_url+result_dict.data_path, '#hspred_initial_model', false, ractive);
      }
      if(second_match)
      {
          ...
          display_structure(file_url+result_dict.data_path, '#hspred_second_model', false, ractive);
      }
    }
    ...
    if(result_dict.name === 'memembed')
    {
      ...
      process_file(file_url, result_dict.data_path, 'zip', zip, ractive);
      display_structure(file_url+result_dict.data_path, '#memembed_model', false, ractive);
      ractive.set('memembed_pdb', file_url+result_dict.data_path);
    }
    if(result_dict.name === 'run_dmpfold')
    {
        let match = dmpfold_regex.exec(result_dict.data_path);

        if(match)
        {
          results_found.dmpfold = true;
          ractive.set("dmpfold_waiting_message", '');
          ractive.set("dmpfold_waiting_icon", '');
          ractive.set("dmpfold_time", '');
          if(result_dict.data_path.startsWith('/submissions/final_1'))
          {
            downloads_info.dmpfold.pdb = '<a href="'+file_url+result_dict.data_path+'">PDB File</a><br />';
            process_file(file_url, result_dict.data_path, 'zip', zip, ractive);
            display_structure(file_url+result_dict.data_path, '#dmpfold_model', true, ractive);
            ractive.set('dmpfold_pdb', file_url+result_dict.data_path);
          }
          else {
            console.log(result_dict.data_path)
            downloads_info.dmpfold.extras += '<a href="'+file_url+result_dict.data_path+'">Additional PDB Model</a><br />';
          }
        }
    }
    if(result_dict.name === 'predict_tmscore')
    {
        downloads_info.dmpfold.tmscore = '<a href="'+file_url+result_dict.data_path+'">TM Score Estimate</a>';
        process_file(file_url, result_dict.data_path, 'zip', zip, ractive);
        ractive.set('dmpfold_tmscore', file_url+result_dict.data_path);
    }
    if(result_dict.name === 'metacov_pred')
    {
      // see plot_self_contact_map
      // see metapsicov_stage2
      results_found.dmp = true;
      ractive.set("dmp_waiting_message", '');
      ractive.set("dmp_waiting_icon", '');
      ractive.set("dmp_time", '');
      downloads_info.dmp.con = '<a href="'+file_url+result_dict.data_path+'">Contact Prediction</a><br />';
      process_file(file_url, result_dict.data_path, 'zip', zip, ractive);
    }
    if(result_dict.name === 'plot_dmp')
    {
      downloads_info.dmp.map = '<a href="'+file_url+result_dict.data_path+'">Contact Map</a><br />';
      process_file(file_url, result_dict.data_path, 'zip', zip, ractive);
      ractive.set('dmp_map', '<img src="'+file_url+result_dict.data_path+'" >');
    }
  }
  ...
  job_list.forEach(function(job_name){
    ...
    {
    ractive.set(job_name+"_waiting_message", 'The job completed succesfully but no confident positive predictions were produced for your input data. More marginal predictions, if detected, can be found in the download results files');
...
export function display_structure(uri, css_id, cartoon, ractive)
{
  ...
  // Keep the panel viewers for table manipulation
  if(css_id === '#metsite_model')
  {
    ractive.set('metsite_mol_viewer', viewer);
  }
  if(css_id === '#hspred_initial_model')
  {
    ractive.set('hspred_initial_mol_viewer', viewer);
  }
  if(css_id === '#hspred_second_model')
  {
    ractive.set('hspred_second_mol_viewer', viewer);
  }
}
...
export function set_downloads_panel(ractive, downloads_info)
{
  ...
  if('s4pred' in downloads_info)
  {
    //if(downloads_info.s4pred.horiz){
    if(downloads_info.s4pred.ss2){
    downloads_string = downloads_string.concat(downloads_info.s4pred.header);
    downloads_string = downloads_string.concat(downloads_info.s4pred.horiz);
    downloads_string = downloads_string.concat(downloads_info.s4pred.ss2);
    downloads_string = downloads_string.concat("<br />");}
  }
  ...
  if('dmpfold' in downloads_info)
  {
    downloads_string = downloads_string.concat(downloads_info.dmpfold.header);
    downloads_string = downloads_string.concat(downloads_info.dmpfold.pdb);
    downloads_string = downloads_string.concat(downloads_info.dmpfold.extras);
    downloads_string = downloads_string.concat(downloads_info.dmpfold.tmscore);
    downloads_string = downloads_string.concat("<br />");
  }
  if('dmp' in downloads_info)
  {
    downloads_string = downloads_string.concat(downloads_info.dmp.header);
    downloads_string = downloads_string.concat(downloads_info.dmp.con);
    downloads_string = downloads_string.concat(downloads_info.dmp.map);
    downloads_string = downloads_string.concat(downloads_info.dmp.psicov);
    downloads_string = downloads_string.concat(downloads_info.dmp.freecontact);
    downloads_string = downloads_string.concat(downloads_info.dmp.ccmpred);
    downloads_string = downloads_string.concat("<br />");
  }
...
export function set_advanced_params()
{
  ...
  try{
    options_data.bioserf_domserf_modeller_key = document.getElementById("bioserf_domserf_modeller_key").value;
  }
  catch(err){
    options_data.bioserf_domserf_modeller_key = "";
  }
  try{
