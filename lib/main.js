...
job_list.forEach(function(job_name, i){
  ...
  ractive.on(job_name+'_active', function( event ){
    ...
    if(job_name === "s4pred")
    {
      if(ractive.get('s4pred_horiz'))
      {
        biod3.psipred(ractive.get('s4pred_horiz'), 's4predChart', {parent: 'div.s4pred_cartoon', margin_scaler: 2});
      }
    }
...
if(getUrlVars().uuid && uuid_match)
{
  ...
  if(previous_data.jobs.includes('s4pred'))
  {
      let config = send_request(joblist_url+'s4pred','GET',{});
      csv = ractive.get('config_text');
      csv += parse_config(config);
      ractive.set('config_text', csv);
      ractive.set('s4pred_results_visible', true);
      ractive.set('results_visible', 2 );
  }
