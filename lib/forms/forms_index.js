  ...
  if(seq_type){
    ...
    if(error_message.length === 0){
      ...
      if(seq_count > 1){
        ...
        data = msa;
        console.log(data);
        active.set('sequence_length', seqs["1"].length);
        ractive.set('subsequence_stop', seqs["1"].length);
        ractive.set('msa', msa);
        ractive.set('sequence', seqs["1"]);
      }
      //check if MSA if yes take first sequence out and add to ractive sequence VARIABLES
      //and renumber the bits
